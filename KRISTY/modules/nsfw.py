...
from KRISTY import dispatcher, updater
import KRISTY.modules.sql.nsfw_sql as sql
...
def add_nsfw(update: Update, context: CallbackContext):
    ...
    if not is_nsfw:
        ...
        msg.reply_text("ᴀᴄᴛɪᴠᴀᴛᴇᴅ ɴꜱꜰᴡ ᴍᴏᴅᴇ ʙᴀʙʏ🖤!")
        message = (
        ...
        msg.reply_text("ɴꜱꜰᴡ ᴍᴏᴅᴇ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ꜰᴏʀ ᴛʜɪꜱ ᴄʜᴀᴛ ʙᴀʙʏ🖤!")
        return ""
...
def rem_nsfw(update: Update, context: CallbackContext):
    ...
    is_nsfw = sql.is_nsfw(chat.id)
    if not is_nsfw:
        msg.reply_text("ɴꜱꜰᴡ ᴍᴏᴅᴇ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴅᴇᴀᴄᴛɪᴠᴀᴛᴇᴅ ʙᴀʙʏ🖤")
        return ""
    else:
        sql.rem_nsfw(chat.id)
        msg.reply_text("ʀᴏʟʟᴇᴅ ʙᴀᴄᴋ ᴛᴏ ꜱꜰᴡ ᴍᴏᴅᴇ ʙᴀʙʏ🖤!")
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"ᴅᴇᴀᴄᴛɪᴠᴀᴛᴇᴅ_ɴꜱꜰᴡ\n"
            f"<b>ᴀᴅᴍɪɴ:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
def list_nsfw_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>ɴꜱꜰᴡ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ᴄʜᴀᴛꜱ ʙᴀʙʏ🖤</b>\n"
    for chat in chats:
        try:
...
def keta(update, context):
    ...
    target = 'keta'
    if not target:
        msg.reply_text("ɴᴏ ᴜʀʟ ᴡᴀꜱ ʀᴇᴄᴇɪᴠᴇᴅ ꜰʀᴏᴍ ᴛʜᴇ ᴀᴘɪ ʙᴀʙʏ🖤!")
        return
    msg.reply_photo(nekos.img(target))
...
kristu_HANDLER = CommandHandler("kristu", kristu, run_async=True)
...
dispatcher.add_handler(kristu_HANDLER)
...
*ɴꜱꜰᴡ:*
» `/addnsfw` : ᴇɴᴀʙʟᴇ ɴꜱꜰᴡ ᴍᴏᴅᴇ
» `/rmnsfw` : ᴅɪꜱᴀʙʟᴇ ɴꜱꜰᴡ ᴍᴏᴅᴇ 

*ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴏᴍᴍᴀɴᴅꜱ:*
» `/neko`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜱꜰᴡ ɴᴇᴋᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/feet`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴀɴɪᴍᴇ ꜰᴇᴇᴛ ɪᴍᴀɢᴇꜱ.
» `/yuri`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʏᴜʀɪ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/trap`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴛʀᴀᴘ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/futanari`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜰᴜᴛᴀɴᴀʀɪ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/hololewd`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʜᴏʟᴏ ʟᴇᴡᴅꜱ.
» `/lewdkemo`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴋᴇᴍᴏ ʟᴇᴡᴅꜱ.
» `/sologif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜱᴏʟᴏ ɢɪꜰꜱ.
» `/cumgif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴄᴜᴍ ɢɪꜰꜱ.
» `/kristu`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴋʀɪꜱᴛɪ ɪᴍᴀɢᴇꜱ.
» `/lesbian`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʟᴇꜱ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/lewdy`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴋɪᴛꜱᴜɴᴇ ʟᴇᴡᴅꜱ.
» `/ngif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ɴᴇᴋᴏ ɢɪꜰꜱ.
» `/tickle`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴛɪᴄᴋʟᴇ ɢɪꜰꜱ.
» `/lewd`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʟᴇᴡᴅꜱ.
» `/feed`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜰᴇᴇᴅɪɴɢ ɢɪꜰꜱ.
» `/eroyuri`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ-ʏᴜʀɪ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/eron`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ-ɴᴇᴋᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/cum`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴄᴜᴍ ɪᴍᴀɢᴇꜱ.
» `/bjgif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʙʟᴏᴡ ᴊᴏʙ ɢɪꜰꜱ.
» `/bj`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʙʟᴏᴡ ᴊᴏʙ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/nekonswf`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ɴꜱꜰᴡ ɴᴇᴋᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/solo`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ɴꜱꜰᴡ ɴᴇᴋᴏ ɢɪꜰꜱ.
» `/kemonomimi`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴋᴇᴍᴏɴᴏᴍɪᴍɪ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/avatarlewd`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴀᴠᴀᴛᴇʀ ʟᴇᴡᴅ ꜱᴛɪᴄᴋᴇʀꜱ.
» `/gasm`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴏʀɢᴀꜱᴍ ꜱᴛɪᴄᴋᴇʀꜱ.
» `/poke`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴘᴏᴋᴇ ɢɪꜰꜱ.
» `/anal`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴀɴᴀʟ ɢɪꜰꜱ.
» `/hentai`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʜᴇɴᴛᴀɪ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/avatar`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴀᴠᴀᴛᴀʀ ꜱᴛɪᴄᴋᴇʀꜱ.
» `/erofeet`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ-ꜰᴇᴇᴛ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/holo`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʜᴏʟᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/tits`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴛɪᴛꜱ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/pussygif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴘᴜꜱꜱʏ ɢɪꜰꜱ.
» `/holoero`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ-ʜᴏʟᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/pussy`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴘᴜꜱꜱʏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/hentaigif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʜᴇɴᴛᴀɪ ɢɪꜰꜱ.
» `/classic`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴄʟᴀꜱꜱɪᴄ ʜᴇɴᴛᴀɪ ɢɪꜰꜱ.
» `/kuni`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴘᴜꜱꜱʏ ʟɪᴄᴋ ɢɪꜰꜱ.
» `/waifu`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴡᴀɪꜰᴜ ꜱᴛɪᴄᴋᴇʀꜱ.
» `/kiss`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴋɪꜱꜱɪɴɢ ɢɪꜰꜱ.
» `/femdom`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜰᴇᴍᴅᴏᴍ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/cuddle`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴄᴜᴅᴅʟᴇ ɢɪꜰꜱ.
» `/erok`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ-ᴋɪᴛꜱᴜɴᴇ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/foxgirl`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜰᴏxɢɪʀʟ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/titsgif`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴛɪᴛꜱ ɢɪꜰꜱ.
» `/ero`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴇʀᴏ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
» `/smug`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ꜱᴍᴜɢ ɢɪꜰꜱ.
» `/baka`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ʙᴀᴋᴀ ꜱʜᴏᴜᴛ ɢɪꜰꜱ.
» `/dva`: ꜱᴇɴᴅꜱ ʀᴀɴᴅᴏᴍ ᴅ.ᴠᴀ ꜱᴏᴜʀᴄᴇ ɪᴍᴀɢᴇꜱ.
"""
