...
from KRISTY import telethn, dispatcher
...
def debug(update: Update, context: CallbackContext):
    ...
    if len(args) > 1:
        if args[1] in ("yes", "on"):
            DEBUG_MODE = True
            message.reply_text("ᴅᴇʙᴜɢ ᴍᴏᴅᴇ ɪꜱ ɴᴏᴡ ᴏɴ ʙᴀʙʏ🖤.")
        elif args[1] in ("no", "off"):
            DEBUG_MODE = False
            message.reply_text("ᴅᴇʙᴜɢ ᴍᴏᴅᴇ ɪꜱ ɴᴏᴡ ᴏꜰꜰ ʙᴀʙʏ🖤.")
    else:
        if DEBUG_MODE:
            message.reply_text("ᴅᴇʙᴜɢ ᴍᴏᴅᴇ ɪꜱ ᴄᴜʀʀᴇɴᴛʟʏ ᴏɴ ʙᴀʙʏ🖤.")
        else:
            message.reply_text("ᴅᴇʙᴜɢ ᴍᴏᴅᴇ ɪꜱ ᴄᴜʀʀᴇɴᴛʟʏ ᴏꜰꜰ ʙᴀʙʏ🖤.")
