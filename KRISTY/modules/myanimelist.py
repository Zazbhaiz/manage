...
def anime(update: Update, context: CallbackContext):
    ...
    try:
        ...
        msg.reply_text("ᴇʀʀᴏʀ ᴄᴏɴɴᴇᴄᴛɪɴɢ ᴛᴏ ᴛʜᴇ ᴀᴘɪ. ᴘʟᴇᴀꜱᴇ ᴛʀʏ ᴀɢᴀɪɴ ʙᴀʙʏ🖤!")
        return ""
    try:
    ...
    if res:
        ...
        msg.reply_text("ɴᴏ ʀᴇꜱᴜʟᴛꜱ ꜰᴏᴜɴᴅ!")
        return
    rep = f"<b>{title} ({japanese})</b>\n"
    rep += f"<b>ᴛʏᴘᴇ:</b> <code>{type}</code>\n"
    rep += f"<b>ꜱᴏᴜʀᴄᴇ:</b> <code>{source}</code>\n"
    rep += f"<b>ꜱᴛᴀᴛᴜꜱ:</b> <code>{status}</code>\n"
    rep += f"<b>ɢᴇɴʀᴇꜱ:</b> <code>{genres}</code>\n"
    rep += f"<b>ᴇᴘɪꜱᴏᴅᴇꜱ:</b> <code>{episodes}</code>\n"
    rep += f"<b>ᴅᴜʀᴀᴛɪᴏɴ:</b> <code>{duration}</code>\n"
    rep += f"<b>ꜱᴄᴏʀᴇ:</b> <code>{score}</code>\n"
    rep += f"<b>ꜱᴛᴜᴅɪᴏ(s):</b> <code>{studios}</code>\n"
...
def manga(update: Update, context: CallbackContext):
    ...
    if res:
        ...
        rep += f"<b>ɢᴇɴʀᴇꜱ:</b> <code>{genres}</code>\n"
        rep += f"<b>ꜱᴄᴏʀᴇ:</b> <code>{score}</code>\n"
        rep += f"<b>ᴠᴏʟᴜᴍᴇꜱ:</b> <code>{volumes}</code>\n"
        rep += f"<b>ᴄʜᴀᴘᴛᴇʀꜱ:</b> <code>{chapters}</code>\n\n"
        rep += f"<a href='{image}'>\u200c</a>"
        rep += f"<i>{synopsis}</i>"
        keyb = [[InlineKeyboardButton("More Information", url=url)]]
