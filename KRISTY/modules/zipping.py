...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴢɪᴘ ꜰɪʟᴇ ʙᴀʙʏ🖤.")
        return
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
    ...
    if event.reply_to_msg_id:
        ...
        await event.reply("ᴜɴᴢɪᴘᴘɪɴɢ ɴᴏᴡ ʙᴀʙʏ🖤...")
            ...
            if os.path.exists(single_file):
                ...
                try:
                    ...
                    await client.send_message(
                        event.chat_id,
                        "{} ᴄᴀᴜꜱᴇᴅ `{}` ʙᴀʙʏ🖤".format(caption_rts, str(e)),
                        reply_to=event.message.id,
                    )
                    continue
...
__help__ = """
 » `/zip` <text> :  ᴢɪᴘ ᴀɴʏ ɢɪᴠᴇɴ ꜰᴏʟᴅᴇʀ
 » `/unzip` <text> :  ᴜɴᴢɪᴘꜱ ᴀɴʏ ɢɪᴠᴇɴ ᴢɪᴘ ꜰᴏʟᴅᴇʀ
 """
