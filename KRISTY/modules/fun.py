...
import KRISTY.modules.fun_strings as fun_strings
...
def runs(update: Update, context: CallbackContext):
    ...
    if update.effective_user.id == 1820525265:
        temp = "ʀᴜɴ ᴇᴠᴇʀʏᴏɴᴇ, ᴛʜᴇʏ ᴊᴜꜱᴛ ᴅʀᴏᴘᴘᴇᴅ ᴀ ʙᴏᴍʙ 💣💣"
...
def sanitize(update: Update, context: CallbackContext):
    ...
    reply_animation(GIF_ID, caption=f"*ꜱᴀɴɪᴛɪᴢᴇꜱ {name}*")
...
def sanitize(update: Update, context: CallbackContext):
    ...
    reply_animation(random.choice(fun_strings.GIFS), caption=f"*ꜱᴀɴɪᴛɪᴢᴇꜱ {name}*")
...
def slap(update: Update, context: CallbackContext):
    ...
    if update.effective_user.id == 1096215023:
        temp = "@KRISTY_MF scratches {user2} ʙᴀʙʏ🖤"
