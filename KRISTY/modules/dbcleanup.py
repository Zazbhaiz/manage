...
import KRISTY.modules.sql.global_bans_sql as gban_sql
import KRISTY.modules.sql.users_sql as user_sql
from KRISTY import DEV_USERS, OWNER_ID, dispatcher
...
def get_invalid_chats(update: Update, context: CallbackContext, remove: bool = False):
    ...
    for chat in chats:
        ...
        if ((100 * chats.index(chat)) / len(chats)) > progress:
            progress_bar = f"{progress}% completed in getting invalid chats ʙᴀʙʏ🖤."
            if progress_message:
                try:
                    bot.editMessageText(
                        progress_bar,
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    msg.reply_text("Getting invalid chat count  ʙᴀʙʏ🖤...")
    invalid_chat_count = get_invalid_chats(update, context)

    msg.reply_text("Getting invalid gbanned count  ʙᴀʙʏ🖤...")
    invalid_gban_count = get_invalid_gban(update, context)

    reply = f"Total invalid chats - {invalid_chat_count} ʙᴀʙʏ🖤\n"
    reply += f"Total invalid gbanned users - {invalid_gban_count} ʙᴀʙʏ🖤"
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in admin_list:
            bot.editMessageText("ʟᴇᴀᴠɪɴɢ ᴄʜᴀᴛꜱ ʙᴀʙʏ🖤 ...", chat_id, message.message_id)
            chat_count = get_muted_chats(update, context, True)
            bot.sendMessage(chat_id, f"ʟᴇꜰᴛ {chat_count} ᴄʜᴀᴛꜱ ʙᴀʙʏ🖤.")
        else:
        ...
        if query.from_user.id in admin_list:
            ...
            reply = "ᴄʟᴇᴀɴᴇᴅ ᴜᴘ {} ᴄʜᴀᴛꜱ ᴀɴᴅ {} ɢʙᴀɴɴᴇᴅ ᴜꜱᴇʀꜱ ꜰʀᴏᴍ ᴅʙ ʙᴀʙʏ🖤.".format(
                invalid_chat_count,
                invalid_gban_count,
            )
