...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./KRISTY/KRISTYFont/logofonts/*")
    ...
    fname="LogoMakeBy_KRISTY.png"
    img.save(fname, "png")
    await tbot.send_file(event.chat_id, file=fname, caption="ᴍᴀᴅᴇ ʙʏ [KRISTY](https://t.me/MISS_KRISTY_BOT) ʙᴀʙʏ🖤")         
    await pesan.delete()
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./KRISTY/KRISTYFont/blackbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "black"
...
𝗪𝗘 𝗛𝗔𝗩𝗘 𝗔𝗗𝗗𝗘𝗗 100+ 𝗙𝗢𝗡𝗧𝗦 𝗙𝗢𝗥 𝗜𝗧

 » `/ylogo` <text> :  Create your logo with your name in Yellow colour 
 » `/vlogo` <text> :  Create your logo with your name in DarkMagenta colour 
 » `/wlogo` <text> :  Create your logo with your name in White colour 
 » `/logo` <text> :  Create your logo with your name with multiple backgroung 
 » `/blogo` <text> :  Create your logo with your name in Blue colour 
 » `/rlogo` <text> :  Create your logo with your name in Red colour 
 » `/alogo` <text> :  Create your logo with your name in Aqua colour 
 » `/glogo` <text> :  Create your logo with your name in Green colour 
"""
