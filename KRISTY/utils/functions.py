...
from KRISTY import aiohttpsession as aiosession
from KRISTY.ex_plugins.dbfunctions import start_restart_stage
from KRISTY.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "KRISTY"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("KRISTY/KRISTYFont/logofont", 55)
    file = f"KRISTY/KRISTYFont/{randint(1000, 9999)}.jpg"
